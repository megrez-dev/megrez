...
func listArticlesByCategory(c *gin.Context) {
	...
	category, err := DAO.GetCategoryBySlug(categorySlug)
	if err != nil {
		c.Redirect(500, "/error")
	}
	articlePOs, err := DAO.ListArticlesByCategoryID(category.ID, pageNum, pageSize)
	if err != nil {
		c.Redirect(500, "/error")
	}
	...
	articlesNum, err := DAO.CountArticlesByCategoryID(category.ID)
	if err != nil {
		c.Redirect(500, "/error")
	}
